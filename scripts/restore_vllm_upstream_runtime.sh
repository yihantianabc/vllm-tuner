...
LEGACY_PATCHED_SHA256="ed1b8dc7816a48b69710631e67d929f6d4c3870ce868f422cd820389bd08731c"
...
if [[ "${CURRENT_SHA256}" == "${UPSTREAM_SHA256}" ]]; then
  echo "vLLM ${VLLM_VERSION} Scheduler is already clean upstream."
  exit 0
fi
if [[ "${CURRENT_SHA256}" != "${LEGACY_PATCHED_SHA256}" ]]; then
  echo "Refusing to modify an unknown Scheduler source: ${CURRENT_SHA256}" >&2
  exit 2
fi

patch --batch --reverse --directory "${SITE_PACKAGES_DIR}" -p1 < "${PATCH_FILE}"
CURRENT_SHA256="$(sha256sum "${SCHEDULER_FILE}" | awk '{print $1}')"
if [[ "${CURRENT_SHA256}" != "${UPSTREAM_SHA256}" ]]; then
  echo "Restored Scheduler checksum mismatch: ${CURRENT_SHA256}" >&2
  exit 2
fi

echo "Restored clean upstream vLLM ${VLLM_VERSION} Scheduler."
